import React, { Component } from 'react';
import "./JobsComponent.css";
import Modal from "../Modal/Modal";
import heart from "../../media/heart.png";
import heartFull from "../../media/heartFull.png";
export default class JobsComponent extends Component {
    constructor(props) {
        super(props);
        this.state={
            showModal: false,
            heartIcon: heart
        }
    }

    jobOfferClickHandler = () => {
        this.setState({
            showModal: true
        })
    };

    jobOfferCloseHandler = () => {
        this.setState({
            showModal: false
        })
    };

    deleteJob = () => {
        const job = this.props.job;
        const deleteCallback = this.props.deleteCallback;
        deleteCallback(job.id);
    };

    mouseEnterHeart = () => {
        this.setState({
            heartIcon: heartFull
        })
    };

    mouseLeaveHeart = () => {
        this.setState({
            heartIcon: heart
        })
    };

    handleHeartClick = () => {
        console.log("TODO handle heart click");
    };

    render() {
        const job = this.props.job;
        return (
            <div id="jobOfferContainer">
                <div id="jobOfferHeaderIcons">
                    <div id="likeJob">
                        <img id="likeIcon" onMouseEnter={this.mouseEnterHeart} onMouseLeave={this.mouseLeaveHeart} onClick={this.handleHeartClick} src={this.state.heartIcon} />
                    </div>
                    <div id="deleteJob">
                        <img id="deleteIcon" onClick={this.deleteJob} src="data:image/svg+xml;utf8;base64," />
                    </div>
                </div>

                {/*TODO add correct link*/}
                <div id="jobImage"><img src={"https://upload.wikimedia.org/wikipedia/commons/6/6f/HP_logo_630x630.png"}/></div>
                <div id="jobTitle" onClick={this.jobOfferClickHandler}>{job.title}</div>
                <div id="jobDescription">{job.phoneNumber}</div>
                {this.state.showModal &&
                <Modal
                    job={job}
                    showModal={this.state.showModal}
                    closeModal={this.jobOfferCloseHandler}
                    saveCallback={this.props.saveCallback}
                />
                }
            </div>
        );
    }
}